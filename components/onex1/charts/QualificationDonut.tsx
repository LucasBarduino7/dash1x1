...
import type { LeadStats } from '@/lib/onex1/types';
  ...
  return (
    <div className="relative">
      ...
      <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
        <span className="text-2xl font-semibold text-zinc-900 tabular-nums">{num(stats.qualificados)}</span>
        <span className="text-[10px] uppercase tracking-wider text-zinc-500">donos de agência</span>
        <span className="mt-1 text-xs text-zinc-500">de {num(stats.total)} leads</span>
      </div>
      <ul className="mt-3 grid grid-cols-2 gap-2 text-xs">
        {data.map((d) => (
          <li key={d.name} className="flex items-center gap-2 rounded-lg bg-white px-2.5 py-1.5">
            <span className="h-2.5 w-2.5 rounded-full" style={{ background: d.color }} />
            <span className="flex-1 text-zinc-700">{d.name}</span>
            <span className="tabular-nums text-zinc-900">{num(d.value)}</span>
          </li>
        ))}
      </ul>
    </div>
